...
not the primary reason differential testing is not more widely used,
however, even ignoring the cost of developing independent
implementations (which may now be mitigated by AI code synthesis).
Rather, many developers do not use differential testing methods
because in practice, the problem is seldom one of comparing two
implementations with identical behavior.  Differential
testing harnesses such as that for the widely used Pyfakefs Python
mock filesystem~\cite{TSTLHarness} often devote up to half their
code to allowance for divergences between a tested and reference
system.  Moreover, the intellectual effort required to construct such
