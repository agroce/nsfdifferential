...
Differential testing~\cite{Differential} has long been considered one of the most powerful
...
its behavior matches function $g$ that is \emph{an (ideally independent)
...
to cryptographic algorithm and protocol implementations.  Since file systems may rest on different
hardware substrates, and compilers may make different choices where a
...
and allowed way, making the full specification of the complex
structure $P(x)
\Rightarrow O_f(f(I_f(x))) = O_g(g(I_g(x)))$, where the guard $P$ and
$O$ and $I$ abstraction functions may also be quite complex.

Despite these difficulties, differential testing is extremely
...
This means that differential testing via separate implementations of
...
Furthermore, one pre-AI way to work around the cost of a second implementation, of
...
cost\footnote{We believe this wil be especially true if LLMs have
  ...
  harnesses, which is one of the goals of this proposal.}.  Therefore,
differential testing (and fuzzing) is \emph{much more practical and
  widely applicable in an era of agentic coding.}

Second, differential approaches seem ideal for probing ambiguities in
prompts/specifications presented to AIs, and AI misunderstandings: if
...
Agentic coders can automatically refer ambiguities they cannot
resolve, discovered during their own automatic testing of multiple
efforts to satisfy a specification, to human judges.  Therefore,
differential testing (and fuzzing) is \emph{also a potentially
  critical technology for ensuring AI-generated code actually
  satisfies fully specified human goals.}

Differential fuzzing is, of course, already an important practice,
particularly in security-critical domains and core systems infrastructure development.  However,
...
tested differentially for a small number of inputs, the ``final outputs''
of fuzzing runs on the two separate systems; this misses out on the
...
in isolation, but represents a unique \emph{relationship between coverage patterns} for a
...
coverage of both systems, essentially a program path over the merged
program {\tt r1 =
  f(x); r2 = g(x); assert (r1==r2);}.  This overcomes the above
objections in part, but still in fact leaves much to be desired, in terms of
...
tools; without OO constructs or language-native threads, objects and
...
real lambdas and closures, rather than weaker proxies such as object
dispatch simulations of lambdas, or C's function pointers.
...
the NEZHA framework of Petsios et al.~\cite{nezha} offered a start on
...
This proposal aims to develop practical first-class differential-aware
fuzzing algorithms and highly effective automated and manual methods
for building differential specifications, that is constructing
functions of the form $P(x)
\Rightarrow O_f(f(I_f(x))) = O_g(g(I_g(x)))$.  The underlying logic of
this proposal is that differential testing's potential is, at present,
greatly limited by the lack of such first-class tools for exploiting
the fact that a very large number of software problems can be defined
as doing ``(almost) the same thing'' as another system.  This proposal
aims to address that limitation via \emph{first class differential
  fuzzing} and \emph{first class differential specification}; in
particular the first idea introduces completely new notions of
\emph{relational coverage} and \emph{provenance-based} interestingness
of observed program behaviors.
