...
``ubiquitous'' software artifacts (e.g., compilers, file systems,
...
functionality and operating system contexts that operate with
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\columnwidth]{basic_fuzzing.png}
  \caption{The ``standard fuzzing algorithm''.}
  \label{fig:original}
  \end{figure}
...
high level that essentially applies to all widely-used fuzzers (shown
graphically in Figure~\ref{fig:original}):
...
NEZHA's fundamental contribution was making a tuple
...
go beyond that initial approach by modifying fuzzer internals (implemented on
...
\begin{itemize}
 ...
 behavior of \emph{both} $f$ and $g$ would be seen to be changed, and the
 ...
 the \emph{provenance} of fuzzing inputs, so that \emph{relative} behavior change or lack
 of change can be \emph{observed} and \emph{exploited} as a signal of
 behavioral divergence.  In short, we aim to exploit a new kind of
 signal for fuzzing: \emph{relational coverage} between compared
 programs, where not only coverage itself matters, but the
 \emph{relation between changes in coverage, in terms of both
   existence and magnitude}, given fixed changes in input.

 While we do not have a full, usable, implementation of even our basic
 ideas,
 preliminary results using a very simple non-production quality hand-made
 mutational, coverage-driven  (based on Python's
 trace facilities)
 fuzzer for inputs to a pair of Python AVL tree implementations do show a
 significant (roughly 10\%) improvement in time-to-detect-divergence
 when divergence in response to inputs is used in addition to standard
 AFL++/NEZHA style sequential coverage tuples.  However, it is
 possible the particular bug and design decisions in the alternative
 implementation of AVL trees were unintentionally influenced by our beliefs about
 the importance of relative feedback, and the magnitude of the change
 is probably in large part due to our extremely inefficient
 implementation of the core fuzzing and coverage instrumentation.
 Unfortunately, preliminary results for more realistic benchmarks require
 implementation inside a realistic fuzzer such as AFL++, which is a
 substantial engineering effort, though as reported below, a team of
 two senior NAU undergraduate students is now beginning work on such
 an implementation.

 All of the above proposed differential adaptations of the core
 mutation-driven fuzzing algorithm are derived from this insight that
 once a fuzzing problem is seen as existing between two or more
 targets, this new \emph{relational} notion of coverage becomes
 potentially very valuable.  The exact nature of relational coverage
 concepts can be quite subtle, involving both static and dynamic
 notions of change.

 For instance, the insight behind the third method listed above, which tries to exploit
 ...
 parts of the implementation; the seemingly fixed and uninteresting
 relation between unchanged code regions is in fact non-fixed and
 complex, mediated by data flow from changed regions.  Changes \emph{wholly contained} in divergent
 ...
 shared code.  This idea therefore makes use of relations between the
 differentially compared programs at both the static and dynamic
 level.  If we simplify the model by assuming the changed code
 ...
 behavior.  The conclusion is less confident with more complex non-contiguous
...
Figure~\ref{fig:modified} shows a preliminary sketch of the full set
of proposed possible first-class differential-aware fuzzing algorithm.

\begin{figure}
  \includegraphics[width=0.8\columnwidth]{diff_fuzzing.png}
  \caption{Proposed differential-aware fuzzing algorithm changes.}
  \label{fig:modified}
  \end{figure}
...
likely to be found in obscure, ill-understood parts of the system state space, and the
...
\begin{itemize}
  \item Users expecting a high degree of automation, who do not want
    ...
    \item Users who are willing to devote considerable effort to
      ...
      unit tests.  File system, compiler, and cryptographic algorithm
...
of differential fuzzing takes as input exactly two binaries, and compares
their behavior, with no additional work required by the user.  In some
cases, ``difference'' may even be left as the simple case where one
system crashes or returns an error code and the other does not.

In a few
cases, this is sufficient.  More commonly, however, considerable complexity
...
functions, but develop \emph{automated calibration methods} that use the
ability of fuzzing to produce a statistically large number of
behaviors in order to estimate a good comparison function.  Inferring a change
...
a domain expert.  Moreover, in such settings, there may be a goal of
using the differential harness for multiple pairs of systems, with
some kind of configuration file controlling the details of the
particular specification to be used.  In general, this problem is
complex and unique enough to merit the use of a Domain Specific
Language (DSL)~\cite{mernik2005domain,fowler2010domain}.
...
compare: inorder
\end{code}
\caption{A TSTL differential harness.}
\label{fig:avl}
\end{figure}

In previous work, we developed the TSTL (Template Scripting Testing
Language)~\cite{NFM15,tstlsttt}, which was used for the testing of the
Pyfakefs mock file system mentioned
above~\cite{pyfakefs,TSTLHarness}.  TSTL produces unit tests of Python
programs using a nondeterministic, pool-based, approach, and includes
unique features to support more complex forms of differential testing.  A simple
differential harness for an AVL tree implementation is shown in
Figure~\ref{fig:avl}.  TSTL lets users specify a testing problem
(here, that of testing an AVL tree) in terms of a single system under
test (SUT) and provide mappings between test actions for the SUT and a
reference implementation in the form of transforms that generate
``mirror'' actions to be performed on the reference.  Here, for
example, a new {\tt AVLTree} construction is always mirrored by
creation of an empty Python {\tt set}.  More complex transforms than
mere name changes are also possible, e.g., where inorder traversal of
an AVL's contents (a method call) is mapped to a function call on a
set.  Users can specify when the return values of actions are to be
compared for equivalence, and can provide Python-coded abstraction
functions over inputs or outputs as needed.  The TSTL approach was
designed to enable differential testing in settings such as file
systems where system behavior over a series of stateful actions is to
be compared, not just single function calls.

While the TSTL approach is effective, allowing the complex Pyfakefs
harness, which resulted in discovery and correction of more than 100 bugs in Pyfakefs
(and one bug on Mac OS/BSD {\tt rename}), to consist of only 124
non-comment lines, while supporting full reference comparison of both
Windows and Mac OS/linux filesystems with Pyfakefs mocks, it
also has serious limitations.  For one thing, many problems included
in the general form of the differential problem, $P(x)
\Rightarrow O_f(f(I_f(x))) = O_g(g(I_g(x)))$, are only weakly
supported by TSTL; for example, while TSTL allows users to specify
that some test actions may throw expected exceptions, it does not
provide a way to distinguish exceptions that can be thrown only by
particular versions of the code, which can lead to false negatives in
testing, when an implementation throws an exception that is allowable
in a different implementation, but not in this one (e.g., consider the
case where one data structure implementation throws a Python {\tt
  KeyError} to indicate an item was not found, and another throws a
custom exception.  If the implementation throwing a custom exception
has an actual internal bug that produces a {\tt KeyError} it will not
be detected (since this will happen in the context of a {\tt find}
operation or similar pure operation with no detectable state change
consequences).  Fully identifying all commonly used differential
idioms (e.g., mapping allowed exceptions to different implementations)
and making them available to developers of complex harnesses is a key
goal of this project.  We expect that it is possible in some cases
identification of differential specification idioms may lead to
identification of opportunities to provide better feedback to the
actual fuzzing algorithms (e.g., to inform the fuzzer when behavior
being explored does not satisfy preconditions for one of the
implementations, so is less interesting than inputs that actually
produce executions of all implementations to differentially compare,
and so should be given a lower energy value or ranking in queue).

More importantly, any DSL for differential testing is
likely to either embed a full programming language (as does TSTL) or
be embedded in a full programming language~\cite{fowler2010domain}.
The TSTL approach has benefits, including the elimination of redundant
and complex code for representing nondeterministic choice of test
actions, but in general for most differential fuzzing problems it is
preferable to embed the DSL in a host language.   TSTL's adoption was
always limited by the need to learn a new programming language, a
potentially daunting task; testing and verification systems such as
JUnit~\cite{junit}, GoogleTest~\cite{GoogleTest}, CBMC~\cite{CBMC},
FuzzTest~\cite{FuzzTest}, and DeepState~\cite{DeepState} that ``meet
developers where they are''~\cite{meeting} without requiring users to
learn a new language have always proved more successful and widely adopted.
In particular, most
of the critical target programs for complex differential harnesses are
written in C or C++ (like most systems and low-level software), and
ideally a solution would have the following features:

\begin{enumerate}
\item Easily support testing of C and C++ target code.
  \item Integrate seamlessly with existing tools for expressing
    property-driven fuzzing problems in C and C++, such as Google
    FuzzTest~\cite{FuzzTest} and Trail of Bits'
    DeepState~\cite{DeepState,deepstaterepo}.
  \end{enumerate}

FuzzTest is a promising system to integrate with because it is widely
used and has the institutional credibility of Google behind it; it
also provides a good baseline for non-first-class but known-effective
fuzzing for differential problems.  DeepState, in contrast, both
provides full support for API-call-sequence fuzzing problems like
file system testing (which is unforunately inherently awkward using
FuzzTest's API) and can make use of our improved, differential-aware,
versions of AFL++.

The critical difficulty is how to satisfy these requirements, which
essentially require a DSL embedded within an existing language (for
integration with FuzzTest/DeepState), without losing the power of TSTL's notation to express ``mirroring''
of actions by transforms of calls or actions on a single target
system, which is extremely concise and expressive, and removes many potential sources
of error in writing a differential harness.  In practice, determining
the best way to do this will require substantial effort, but our
preliminary experiments in this direction suggests that a kind of
hybrid DSL approach, with a comment-embedded language (like doxygen, JML,
and other methods that work via semantically-meaningful comments) that
constructs macro definitions that safely apply transforms to seemingly
single-target C and C++ code is one possible solution.  This would mix
the typical FuzzTest/DeepState approach to expressing calls to tested
functions, where such calls simply appear as normal C/C++ code, with
an auto-coding step where a set of TSTL-style {\tt reference} and
other transform indicators map out an expanded version of the harness
to compile and run.  Such mappings could also potentially provide some
support for call-sequences testing for FuzzTest, since a transform
step is already being applied to the fuzzing harness.
...
executions as failing, with no additional effort.  This means that in
automatic calibration of differential specifications, the impetus is
on the fuzzing to explore the system sufficiently to identify
likely-spurious, systematic, differences and present them for user
approval, and in manual harness construction the impetus is on the DSL
to support ways to express qualifications and modifications of the
differential specification that are frequently used, and invoke
abstractions and type transforms in an easily read and easily modified way.
...
GitHub stars (and, in one case, more than 800 stars: DeepState); he has, in the
...
most-cited papers on differential testing~\cite{ICSEDiff} and the TSTL testing
